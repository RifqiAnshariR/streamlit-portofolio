...
with col1:
    ...
    educations = [e for e in EDUCATIONS if e.get("institution")]
    ...
    experiences = [e for e in EXPERIENCES if e.get("role") and e.get("company")]
    ...
    skills = [s for s in SKILLS if s.get("category")]
...
with col2:
    ...
    for certification in certifications:
        st.write(f"{certification.get('name')} — {certification.get('issuer')}")
        date = certification.get('date', '')
        cred_id = certification.get('credential_id', '')
        if date or cred_id:
            caption_parts = []
            if date:
